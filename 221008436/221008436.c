...
int verificadornome(char nome[]){ //verifica se o nome tem pipe ("|"). Se tiver, eh um nome invalido.
    int trulen; //isso aqui serve para acomplar nomes de tamanhos variados
    int i, j; //os clássicos
    for(i = 0; nome[i] != '\0'; i++); //acha o tamanho total da string
    trulen = i;
    int failflag = 0;
    for(i=0; i<= trulen - 1; i++){
        if((int)nome[i] == 124){
...
void cadastrausina(){ //NOMINAL
    ...
    scanf("%s", itaipu.nome);
    char cnpj[14];
    printf("Insira cnpj da usina sem pontos ou tracos:\n");
    scanf("%s", cnpj);
    formatter(cnpj, itaipu.cnpj);
...
void cadastracliente(){ //NOMINAL
    ...
    printf("Insira cpf da pessoa sem pontuacoes.\n");
    scanf("%s", unformatcpf[i]);
    formatter(unformatcpf, cliente.cpf);
...
void gravapessoa(pessoa consumidor){ //NOMINAL
    int i;
    FILE *clientes;
...
void formatter(char unfor[], int format[]){ //formata um string num array. um digito por espaco. //NOMINAL.
    int i, trulen, minusflag;
    for(i = 0; unfor[i] != '\0'; i++);
    trulen = i;
    if(unfor[0] == '-'){
        minusflag = 1;
